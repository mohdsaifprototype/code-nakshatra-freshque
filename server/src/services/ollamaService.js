	...
	substitutions: [
		{
			...
			note: "Ollama is offline — start it to get real suggestions.",
		},
	],
...
Return ONLY valid JSON (no markdown), exactly in this shape:
{
	...
	try {
		...
		if (!parsed || !Array.isArray(parsed.substitutions)) {
			console.warn(
				"[ollamaService] remixRecipe: unexpected response shape, using fallback",
			);
...
const GENERATE_FALLBACK = {
	...
	instructions:
		"Ollama is not running. This is a sample recipe. Start Ollama and click 'Invent new recipe' again to get a real AI-generated dish.",
};
...
Return ONLY valid JSON (no markdown), exactly in this shape:
{
	...
	try {
		...
		if (!parsed || !parsed.title) {
			console.warn(
				"[ollamaService] generateRecipe: unexpected response shape, using fallback",
			);
