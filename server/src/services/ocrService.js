...
function parseReceiptText(rawText) {
  ...
  for (const line of lines) {
    ...
    if (/^(total|subtotal|tax|gst|cgst|sgst|discount|savings|amount|date|bill|invoice|shop|store|thank)/i.test(line)) {
    ...
    const receiptRow = line.match(/^(\d+)?\s*(.+?)\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)$/);
    if (receiptRow) {
      const [, srNo, name, qty, price, total] = receiptRow;
      console.log("[ocr]   -> Pattern 1 match! SrNo:", srNo, "Name:", name, "Qty:", qty, "Price:", price, "Total:", total);
      ...
      if (Math.abs(calcTotal - actualTotal) < 1) {
        ...
        console.log("[ocr]   -> Validation failed: calcTotal", calcTotal, "vs actualTotal", actualTotal);
      }
    } else {
...
function cleanName(raw) {
  return raw.replace(/[^a-zA-Z0-9\s\-&.]/g, "").trim().slice(0, 80);
}

function normaliseUnit(u) {
  const map = { l: "L", ltr: "L", litre: "L", ml: "ml", kg: "kg", g: "g", pcs: "pcs", pack: "pack", dozen: "dozen" };
...
function guessCategory(name) {
  const n = name.toLowerCase()
    ...
    .replace(/aashirvaad|india gate|fortune|tata|amul|britannia|dhara|sundrop|saffola|patanjali|parle|dabur|nestlé|nestle|mdh|everest|catch|maggi|knorr|haldirams|lays|kurkure|real|tropicana|kissan|kissan|mtr|gits|mother dairy|heritage|nandini/gi, "")
    .trim();

  if (/(milk|curd|dahi|paneer|cheese|butter|ghee|cream|lassi|yogurt|khoya|mawa|kadai|mozzarella|condensed)/.test(n)) return "dairy";
  if (/(oil|refined|sunflower|mustard|groundnut|coconut oil|olive|til oil|sesame)/.test(n)) return "oils";
  if (/(tomato|potato|onion|carrot|cabbage|spinach|capsicum|cucumber|bean|pea|brinjal|gourd|pumpkin|banana|apple|mango|orange|grape|lemon|lime|fruit|veggie|vegetable|sabzi|garlic|ginger|beetroot|okra|ladyfinger|bhindi|methi|fenugreek|coriander leaves|mint|curry leaves|drumstick|moringa|yam|arbi|sweet potato)/.test(n)) return "produce";
  if (/(atta|flour|wheat|rice|basmati|oats|suji|maida|poha|semolina|roti|naan|pav|bread|cornflour|besan|gram flour|millet|bajra|jowar|ragi|quinoa|barley|vermicelli|pasta|noodle|sabudana|sago)/.test(n)) return "grains";
  if (/(dal|lentil|chana|rajma|tofu|chicken|mutton|fish|egg|meat|prawn|lamb|beef|soya|soy|moong|masoor|urad|toor|arhar|kabuli|chole|peanut|almond|cashew|walnut|pistachio|dry fruit|badam)/.test(n)) return "protein";
  if (/(salt|pepper|turmeric|masala|haldi|cumin|coriander|chilli|garam|spice|ajwain|jeera|hing|asafoetida|clove|cardamom|cinnamon|bay leaf|star anise|mustard seed|fenugreek seed|fennel|saunf|methi seed|kali mirch|lal mirch|dhaniya|kasuri|amchur|chaat|sambhar masala|rasam|biryani masala|kitchen king|pav bhaji masala|chhole masala)/.test(n)) return "spices";
  if (/(biscuit|cookie|chips|snack|namkeen|popcorn|chocolate|candy|wafer|kurkure|lays|bhujia|mixture|chakli|murukku|mathri|diet khakhra|protein bar|granola bar)/.test(n)) return "snacks";
  if (/(juice|water|soda|cola|coffee|tea|drink|beverage|lassi bottle|nimbooz|frooti|maaza|sprite|pepsi|coke|thums up|energy drink|green tea|herbal tea|chai|cocoa|horlicks|bournvita|boost|complan|glucose|squash|sharbat)/.test(n)) return "beverages";
  if (/(bread|cake|puff|pastry|muffin|bakery|croissant|rusk|toast|khari|bun|donut|doughnut)/.test(n)) return "bakery";
  return "other";
}
