...
const itemSchema = z.object({
  ...
  category: z.enum(["dairy", "produce", "grains", "protein", "spices", "bakery", "snacks", "beverages", "other"]).optional(),
...
pantryRouter.get("/", catchAsync(async (req, res) => {
  const items = await PantryItem.find({ userId: req.user.id, consumed: { $ne: true } })
    .sort({ expiryDate: 1 })
    .lean();
...
pantryRouter.post("/", catchAsync(async (req, res) => {
...
pantryRouter.post("/bulk", catchAsync(async (req, res) => {
...
pantryRouter.patch("/:id", catchAsync(async (req, res) => {
...
pantryRouter.post("/:id/consume", catchAsync(async (req, res) => {
...
pantryRouter.delete("/:id", catchAsync(async (req, res) => {
  const r = await PantryItem.deleteOne({ _id: req.params.id, userId: req.user.id });
